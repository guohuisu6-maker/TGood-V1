...
def login_page():
    ...
    if os.path.exists(login_path):
        with open(login_path, 'r', encoding='utf-8') as f:
            return f.read(), 200, {'Content-Type': 'text/html'}
    return 'Login file not found', 404
...
def add_transaction(stock_id):
    ...
    if tx_type not in ('base', 'buy', 'sell') or price <= 0 or shares <= 0:
...
def delete_transaction(tx_id):
    ...
    if row:
        stock_id, round_num = row
        cursor.execute('DELETE FROM transactions WHERE stock_id = ? AND round_num = ?', (stock_id, round_num))
...
def get_stock_summary(stock_id):
    ...
    market = dict(market_row) if market_row else {'price': 0, 'change': 0}
...
def get_overview():
    ...
    for stock in stocks:
        ...
        for tx in transactions:
            if tx['type'] in ('base', 'buy'):
                if tx['round_num'] not in buy_tx_map:
                    buy_tx_map[tx['round_num']] = []
                buy_tx_map[tx['round_num']].append(tx)
            elif tx['type'] == 'sell':
                if tx['round_num'] not in sell_tx_map:
                    sell_tx_map[tx['round_num']] = []
                sell_tx_map[tx['round_num']].append(tx)
        
        for round_num in buy_tx_map:
            buys = buy_tx_map[round_num]
            sells = sell_tx_map.get(round_num, [])
            buy_idx = 0
            sell_idx = 0
            while buy_idx < len(buys) and sell_idx < len(sells):
                ...
                stock_total_profit += (sell['price'] - buy['price']) * matched_shares
    ...
    return jsonify({
        ...
        'total_hold_shares': total_hold_shares,
...
def import_data():
    ...
    try:
        for stock in data['stocks']:
            cursor.execute('INSERT OR IGNORE INTO stocks (code, name, market) VALUES (?, ?, ?)',
        ...
        for stock in data['stocks']:
            ...
            if row:
                stock_id = row[0]
                for tx in data['transactions'].get(stock['id'], []):
                    cursor.execute('SELECT id FROM transactions WHERE stock_id = ? AND type = ? AND round_num = ?',
                                 (stock_id, tx['type'], tx['round_num']))
                    if not cursor.fetchone():
                        cursor.execute('INSERT INTO transactions (stock_id, type, price, shares, round_num) VALUES (?, ?, ?, ?, ?)',
                                     (stock_id, tx['type'], tx['price'], tx['shares'], tx['round_num']))
