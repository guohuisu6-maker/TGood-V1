...
def init_sample_data():
    ...
    for code, name, market in sample_stocks:
        cursor.execute('INSERT OR IGNORE INTO stocks (code, name, market) VALUES (?, ?, ?)', (code, name, market))
    
    for code, transactions in sample_transactions.items():
        cursor.execute('SELECT id FROM stocks WHERE code = ?', (code,))
        row = cursor.fetchone()
        if row:
            stock_id = row[0]
            for tx_type, price, shares, round_num in transactions:
                cursor.execute('SELECT id FROM transactions WHERE stock_id = ? AND type = ? AND round_num = ?',
                             (stock_id, tx_type, round_num))
...
@app.route('/')
def index():
    with open(os.path.join(RESOURCE_DIR, 'index.html'), 'r', encoding='utf-8') as f:
        return f.read()


@app.route('/login')
def login_page():
    with open(os.path.join(RESOURCE_DIR, 'login.html'), 'r', encoding='utf-8') as f:
        return f.read()
